...
def process_single_pdb_wrapper(args):
    """Wrapper to process a single PDB file."""
    pdb_file, folder_path, output_path, max_file_size_mb = args
    pdb_path = os.path.join(folder_path, pdb_file)
    pdb_id = os.path.splitext(pdb_file)[0]
    try:
        return pdb_id, process_single_pdb(pdb_path, pdb_id, output_path, max_file_size_mb)
    except Exception as e:
        return pdb_id, {"status": "error", "error": str(e)}


def process_pdb_full_features_parallel(folder_path, output_path=None, max_file_size_mb=25, max_workers=None):
...
def process_single_pdb(pdb_path, pdb_id, output_path, max_file_size_mb=25):
    ...
    try:
        ...
        if file_size_mb > max_file_size_mb:
            print(f"Skipping {pdb_id}.pdb - too large ({file_size_mb:.2f} MB)")
            return {"status": "skipped", "size_mb": file_size_mb}

        print(f"Processing {pdb_id}.pdb ({file_size_mb:.2f} MB)")

        structure, residues_by_chain = parse_basic_structure(pdb_path)
        if not structure or not residues_by_chain:
            print(f"Failed to parse {pdb_id}.pdb")
            return {"status": "error", "error": "Failed to parse structure"}
        ...
        print(f"Error processing {pdb_id}.pdb: {str(e)}")
        traceback.print_exc()
...
if __name__ == "__main__":
    folder_path = "pdb_random_proteins"  # Your PDB folder
    output_path = "prots_networkx"
    
    # Process with memory optimizations but full features
    summary = process_pdb_full_features_parallel(
        folder_path,
        output_path=output_path,
